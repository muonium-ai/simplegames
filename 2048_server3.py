...
def screenshot():
    ...
    pygame.image.save(screenshot_image, img_bytes)
...
def main():
    ...
    while True:
        ...
        if game_instance.game_over:
            ...
            draw_text(window, "Game Over!", 64, WINDOW_WIDTH // GAME_SIZE, WINDOW_HEIGHT // 2, TEXT_LIGHT)
        elif game_instance.game_won:
            overlay = pygame.Surface((WINDOW_WIDTH, WINDOW_HEIGHT))
            overlay.fill((255, 223, 0))
            overlay.set_alpha(128)
            window.blit(overlay, (0, 0))
            draw_text(window, "You Win!", 64, WINDOW_WIDTH // GAME_SIZE, WINDOW_HEIGHT // 2, TEXT_DARK)
