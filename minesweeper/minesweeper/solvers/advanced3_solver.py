...
class Solver:
    ...
    def next_move(self):
        ...
        # If CSP solver doesn't find a move, proceed to probabilistic guessing
        move = self.probabilistic_guess()
        if move:
            return move

        # If probabilistic_guess fails, handle no logical steps available
        self.handle_no_logical_steps()
        # Make a random move after handling
        move = self.random_hidden_cell()
        if move:
            x, y = move
            if self.debug_mode:
                print(f"Making a random reveal at ({x}, {y}) after no logical steps")
            return x, y, 'reveal'
        return None  # No moves left

    def handle_no_logical_steps(self):
        """Handle the scenario where no logical steps are available."""
        print("No logical steps available")
        if self.debug_mode:
            print("No logical steps available - proceeding with a random guess.")
    ...
    def probabilistic_guess(self):
        ...
        if best_cell:
            if self.debug_mode:
                print(f"Probabilistic guess at ({best_cell[0]}, {best_cell[1]}) with mine probability: {min_probability}")
    ...
    def csp_solver(self):
        ...
        for y in range(GRID_HEIGHT):  # Corrected loop bounds
            for x in range(GRID_WIDTH):  # Corrected loop bounds
                cell = self.game.grid[y][x]
                if cell.state == CellState.REVEALED and cell.neighbor_mines > 0:
                    hidden_neighbors, flagged_neighbors = self.get_neighbors(x, y)
                    remaining_mines = cell.neighbor_mines - len(flagged_neighbors)
